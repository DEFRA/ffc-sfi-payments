...
const applyCSDualAccounting = (paymentRequests, previousPaymentRequests) => {
  for (const paymentRequest of paymentRequests) {
    ...
    const previousFundCode = previousPaymentRequestsWithInvoiceLines[previousPaymentRequestsWithInvoiceLines.length - 1]?.invoiceLines.filter(x => [DRD00, DRD01, DRD05].includes(x.fundCode))?.fundCode
